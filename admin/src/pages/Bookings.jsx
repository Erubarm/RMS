        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#f8f9fa' }}>
              {['ID', 'Код', 'Экскурсия', 'Дата слота', 'Время', 'Чел.', 'Телефон', 'Статус', 'Действия'].map((h) => (
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={9} style={{ textAlign: 'center', padding: 40, color: '#aaa' }}>Загрузка...</td></tr>
            ) : bookings.length === 0 ? (
              <tr><td colSpan={9} style={{ textAlign: 'center', padding: 40, color: '#aaa' }}>Нет бронирований</td></tr>
            ) : bookings.map((b) => (
              <tr key={b.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '12px 16px' }}>
                  {b.phone ? (
                    <a href={`tel:${b.phone}`} style={{ color: '#1565C0', textDecoration: 'none', fontWeight: 500 }}>
                      {b.phone}
                    </a>
                  ) : '—'}
                </td>
